...
jest.mock('@vscode-marquee/widget-news/extension', () => (
  { activate: () => ({ marquee: { disposable: require('disposableManager') }}) })
)
...
jest.mock('@vscode-marquee/widget-dependencies/extension', () => (
  { activate: () => ({ marquee: { disposable: require('disposableManager') }}) })
)
jest.mock('@vscode-marquee/widget-runme/extension', () => (
  { activate: () => ({ marquee: { disposable: require('disposableManager') }}) })
)
...
test('_import', async () => {
  ...
  expect(manager.setImportInProgress).toBeCalledTimes(28)
})
...
test('clearAll', async () => {
  ...
  expect(manager.clear).toBeCalledTimes(14)
})

test('onWidget', () => {
  const stateManager = new StateManager({ ...context, extensionPath: '/foo/bar' } as any)
  stateManager.onWidget('foobar', () => {})
  expect(manager.on).toBeCalledTimes(14)
